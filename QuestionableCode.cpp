// QuestionableCode.cpp : This file contains the 'main' function.
...
// TODO: Compare the warnings and errors between Visual Studio and CppCheck.
// Make sure you are using CppCheck 2.1 or greater.
//
// 1. Create a Visual Studio Console project with this file as the only file.
// 2. Compile it and check the warnings and errors.
// 3. Create a CppCheck project to analyze a Visual Studio project with this file.
// 4. In CppCheck Edit / Preferences / General Tab:
//    - Set "Display error id in column Id"
//    - Enable inline suppressions
//    - Check for inconclusive errors also
// 5. In CppCheck View, select Check All to ensure all check types are enabled.
// 6. In CppCheck Analyze, set the C++ Standard to C++17 and enforce C++.
// 7. Run the analysis.
// 8. Save the results to a file in XML format.
// 9. Take a screenshot of the Visual Studio Error List.
// 10. Identify all CppCheck messages not identified in Visual Studio.
//     For each message not in both:
//     - Identify the risk as RISK or NOT RISK.
//     - Identify which system found the issue.
...
void work_with_arrays(int count)
{
    ...
    if (count == 1000)
    {
        buf[count] = 0; // ERROR: out-of-bounds write
    }
}
...
int foo(Token* tok)
{
    while (tok)
    {
        // Intentional issue for static analysis demonstration.
    }
